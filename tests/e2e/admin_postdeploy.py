#!/usr/bin/env python3
"""ADMIN post-deploy grants (as ADMIN_PROFILE). These need the app SP (born at deploy)
and account-host rights a regular deployer lacks, so the admin does them once, after deploy.

    python3 -m tests.e2e.admin_postdeploy <in_process|job|job_warehouse>

  in_process → the app SP is the tool-runner, so: (1) make it a MEMBER of the AIA role (it inherits the
               evidence SELECT / tool EXECUTE grants by membership — no role assumption, no minted token),
               (2) grant it warehouse CAN_USE DIRECTLY (it hits the warehouse as itself), and (3) grant it
               ACCESS on the LLM UC service credential (it mints the gateway token at runtime — lib/llm.py).
  job / job_warehouse → nothing here. The job SP is the tool-runner and is known pre-deploy, so
               admin_prereqs already granted it role MEMBERSHIP + LLM-credential ACCESS (+ warehouse
               CAN_USE for job_warehouse); nothing needs the app.
"""
import json
import sys
...
def main(mode: str) -> None:
    ...
    if mode in ("job", "job_warehouse"):
        # Both job variants: the job SP is the tool-runner and is created pre-deploy, so admin_prereqs
        # already granted it role membership + LLM-credential ACCESS (+ warehouse CAN_USE for job_warehouse).
        # Nothing here needs the app to exist.
        print(f"{mode}: tool access via role MEMBERSHIP + credential ACCESS (+ warehouse for job_warehouse) "
              f"granted in 00 — nothing to do.")
        print(f"\nADMIN post-deploy grants done (mode={mode}). Next: USER runs 03_setup.py")
        return
