#!/usr/bin/env python3
"""ADMIN post-deploy grants (as ADMIN_PROFILE). These need the app SP (born at deploy) and account-host
rights a regular deployer lacks, so the admin does them once, after deploy.
  ...
  in_process → the app SP is the tool-runner, so: (1) make it a MEMBER of the AIA role (it inherits the
               ...
               ACCESS on the LLM UC service credential (it mints the gateway token at runtime).
  job / job_warehouse → nothing here. The job SP is the tool-runner and is known pre-deploy, so admin_prereqs
               already granted it role MEMBERSHIP + LLM-credential ACCESS (+ warehouse CAN_USE for
               job_warehouse); nothing needs the app.

(The external-MCP connection/service showcase is a SEPARATE later phase — tests/e2e/showcase_external_mcp.py —
not core post-deploy grants, so it lives on its own.)
"""
import json
import sys
...
def main(mode: str) -> None:
    ...
    if mode in ("job", "job_warehouse"):
        print(f"{mode}: tool access via role MEMBERSHIP + credential ACCESS (+ warehouse for job_warehouse) "
              f"granted in admin_prereqs — nothing to do.")
